...
**Important:** Tables below list **candidates** only. Listing an id here does **not** mean it exists in the user’s account. For templates: `harness_search` / `harness_list` → `harness_get(template_v1)` must succeed → emit `uses: <id>` (add `@versionLabel` only when pinning). If get fails, do **not** emit the cookbook id. Spec actions still require the capability to apply; do not invent names.

## Step Selection Priority

1. **Use a `template:` step** if a step template exists after discovery + successful get
2. **Use an `action:` step** if a Harness spec action covers the task and no template is available
...
```yaml
- template:
    uses: <templateId>            # bare id from successful get (default)
    # uses: <templateId>@<version>  # only when pinning
    with:
      param: value
```
...
| Task | Action / Template (candidates — verify) | Instead of |
|------|-------------------|------------|
...
Only after `harness_get` succeeds for this template:
