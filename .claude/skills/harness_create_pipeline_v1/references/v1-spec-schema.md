...
Stage/step `id` is optional in schema. On create, any node with a runtime input (`<+input>`) needs an `id`, as do its ancestors. `${{ inputs.… }}` alone does not trigger this rule.

```yaml
# Runtime input → stage id required (create fails without it)
- id: deploy
  name: deploy
  delegate: <+input>
  steps:
...
Stage `service` may be a plain id. Stage `environment` must be an **object** (`id`, optional `deploy-to`) — not a plain environment id.
...
Pipeline-level `environment: prod` (plain string) is allowed by the schema. That is **not** the same as stage-level `environment:` — do not copy this shape onto stages.
...
> **Runtime source of truth:** Template input names, defaults, `ui.visible`, `ui.inputs`, and `layout` are **not** defined in this file. Always fetch the live template YAML via `harness_get(resource_type='template_v1', ...)` and parse `response.template.yaml`. Global templates change independently (version labels, visibility rules, column keys). Do not copy field names or values from this reference.

v1 templates may include `template.inputs`, optional `template.layout`, and per-input `ui` metadata. The formal JSON schema (`harness/harness-schema` → `v1/template.json`) only types `ui` as an open object — concrete keys come from each template's YAML.

### Fields to parse at runtime (not hardcoded)

| Path | Meaning |
|------|---------|
| `template.inputs.<name>.type` / `required` / `default` | Input schema |
| `template.inputs.<name>.ui.visible` | Conditional visibility expression — evaluate with defaults from the fetched YAML |
| `template.inputs.<name>.ui.inputs[]` | List/grid column schema — wire `with:` rows using each column's `relativePath` |
| `template.inputs.<name>.ui.tooltip` | Use for pipeline input descriptions |
| `template.layout` | Primary vs advanced (`variant: more`) field ordering — omit advanced unless user asks |

Generic shape (field names vary per template):
...
template:
  inputs:
    <input_name>:
      type: string
      default: <from fetched yaml>
      ui:
        visible: <expression or absent>
        inputs:           # only on list/grid types
          - relativePath: <column_key>
            label: <column label>
  layout:
    - <primary_input>
    - variant: more
